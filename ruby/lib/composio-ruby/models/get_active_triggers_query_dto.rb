...
module ComposioRuby
  class GetActiveTriggersQueryDTO
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ComposioRuby::GetActiveTriggersQueryDTO` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ComposioRuby::GetActiveTriggersQueryDTO`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'trigger_names')
        self.trigger_names = attributes[:'trigger_names']
      end
    ...
    def hash
      [connected_account_ids, integration_ids, trigger_ids, trigger_names, page, limit, show_disabled].hash
    end
